...
# NOT: "kivy" sürüm belirtilmeden yazılmıştı; bu, derleme
# her yapıldığında o anki "en güncel" Kivy sürümünün
# indirilmesi anlamına gelir ve derlemeler arasında (özellikle
# TextInput/hint gibi düşük seviyeli davranışlarda) farklılık
# çıkabilir. Bilinen, kararlı sürüme (2.3.1) sabitlendi.
...
# Not: Android'in kendi açılış (bootstrap) yükleme ekranı,
# özel bir presplash görseli verilmezse varsayılan olarak
# siyah zeminli "Loading..." yazılı Kivy kuşu ikonunu
# gösteriyordu. Kullanıcı isteği üzerine bu ekran kaldırıldı:
# artık düz beyaz bir görsel (presplash_beyaz.png) kullanılıyor,
# böylece açılışta sadece beyaz zemin görünüyor ve hemen
# ardından uygulamanın kendi (yine beyaz zeminli, sadece logolu)
# AcilisEkrani'na geçiliyor. İkisi de aynı beyaz renk olduğu
# için kullanıcı tek, kesintisiz bir beyaz açılış görür.
presplash.filename = %(source.dir)s/presplash_beyaz.png
...
# NOT: Kamera kaldirildigi icin CAMERA izni kullanilmiyor.
# Galeriden secim icin READ_MEDIA_IMAGES / READ_EXTERNAL_STORAGE yeterli.
android.permissions = INTERNET,POST_NOTIFICATIONS,RECEIVE_BOOT_COMPLETED,WAKE_LOCK,SCHEDULE_EXACT_ALARM,FOREGROUND_SERVICE,READ_MEDIA_IMAGES,READ_EXTERNAL_STORAGE,WRITE_EXTERNAL_STORAGE,RECORD_AUDIO

# (list) features (adds uses-feature -tags to manifest)
#android.features = android.hardware.usb.host

# NOT: Kamera ozelligi kaldirildi. FileProvider / extra manifest /
# p4a hook karmasikligina artik gerek yok, sadece galeriden secim
# var (fotograf.py).

# AndroidX destegi: bildirim (androidx.core.app.NotificationCompat)
# icin gerekli, acik kalmali.
...
# kütüphanesini APK'ya eklemez, sadece p4a'nın kendi bootstrap'ini
# AndroidX'e çevirir. bildirimler.py içinde kullanılan
# androidx.core.app.NotificationCompat sınıfı ("Bildirim
# gönderilemedi: ... ClassNotFoundException ... NotificationCompat"
# hatası) ve makbuz paylaşımında kullanılan
# androidx.core.content.FileProvider sınıfı, bu satır olmadan
# APK içinde bulunmuyordu. Bu yüzden her iki hata da buradan
# kaynaklanıyordu.
