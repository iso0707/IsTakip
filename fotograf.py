...
def _galeri_izni_var_mi():
    # HATA DÜZELTMESİ: Uygulama açılışında istenen depolama/
    # galeri izni henüz onaylanmamışsa galeri seçici sessizce
    # başarısız olabiliyordu. Açmadan hemen önce kontrol edip
    # gerekirse yeniden isteriz.
    try:
        ...
        if hasattr(Permission, "READ_MEDIA_IMAGES"):
            izinler.append(Permission.READ_MEDIA_IMAGES)
        else:
            izinler.append(Permission.READ_EXTERNAL_STORAGE)

        if all(check_permission(p) for p in izinler):
            return True

        request_permissions(izinler)
        return False

    except Exception as e:
        print(f"[fotograf] izin kontrolü yapılamadı: {e}")
        return True
...
class FotografSecici(BoxLayout):
    ...
    def _galeri_ac(self, *args):
        ...
        if not _galeri_izni_var_mi():
            self.durum_label.text = (
                "Galeri izni isteniyor, izin "
