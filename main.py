...
MENU_IKON_RENKLERI = {
    "malzeme": (0.10, 0.35, 0.92, 1),
    "yeni": (0.02, 0.60, 0.42, 1),
    "gelir": (0.85, 0.62, 0.08, 1),
    "rapor": (0.52, 0.24, 0.86, 1),
    "gecmis": (0.82, 0.16, 0.24, 1),
    "ayar": (0.36, 0.42, 0.52, 1)
}
...
class IkonKare(BoxLayout):

    # Menü kartlarındaki renkli, yuvarlak köşeli küçük
    # ikon kutusu (📦, 🛒, ₺, 📊, 🧾, ⚙ gibi bir emoji /
    # sembol içerir).
...
class MenuKarti(ButtonBehavior, BoxLayout):
    ...
    # Ana menüdeki 2 sütunlu ızgarada yer alan, sol
    # tarafta renkli ikon kutusu, ortada başlık, sağda
    # ">" oku olan koyu kart.
...
class AltMenuDugmesi(ButtonBehavior, BoxLayout):

    # Alt gezinme çubuğundaki tek bir ikon (Ana Sayfa,
    # Malzeme, Yeni İş, Gelir/Gider, Geçmiş, Ayarlar).
        ...
        **kwargs
        ...
        with self.canvas.before:
            ...
            if aktif:
                ...
                self.bind(
                    pos=self._cizgi_guncelle,
                    size=self._cizgi_guncelle
                )

    def _cizgi_guncelle(self, *args):
    ...
    def on_release(self):
        ...
        if self.ekran and self.ekran != "ana":
            _ekrana_git(self, self.ekran)


class AltMenuCubugu(BoxLayout):

    # Ekranın en altındaki, uygulama içindeki ana
    # bölümlere hızlı geçiş sağlayan ikon çubuğu.
    ...
    def __init__(
        ...
        **kwargs
        ...
        for emoji, ekran, aktif in dugmeler:
...
class AnaSayfa(Screen):
    ...
    def __init__(
        ...
        **kwargs
        ...
        ust_satir.add_widget(Widget())

        self.kar_kart.add_widget(ust_satir)
        ...
        self.kar_kart.add_widget(self.kar_baslik)
        ...
        self.kar_kart.add_widget(self.kar_tutar)
        ...
        self.kar_kart.add_widget(self.kar_alt_satir)

        ana.add_widget(self.kar_kart)
        ...
        menuler = [
            ("📦", "malzeme", "Malzeme / Ödemeler"),
            ("🛒", "yeni", "Yeni İş"),
            ("₺", "gelir", "Gelir / Gider"),
            ("📊", "rapor", "Raporlar / Grafikler"),
            ("🧾", "gecmis", "Geçmiş İşler"),
            ("⚙", "ayar", "Yedekleme / Ayarlar")
        ]

        for emoji, ekran, yazi in menuler:
    ...
    def on_enter(self):
        ...
        if toplam_alinacak > 0:
            ...
            self.kar_kart.renkleri_ayarla(
                ALACAK_KART_KOYU,
                ALACAK_KART_ACIK
            )

            self.kar_baslik.text = "Alınacak"

            self.kar_tutar.text = (
                f"{tl_bicimle(toplam_alinacak)} TL"
            )

            self.kar_tutar.color = ALACAK_YAZI

            self.kar_alt_satir.text = (
                "🔴 Tahsilat bekleniyor"
            )

            self.kar_alt_satir.color = ALACAK_YAZI
            ...
            self.kar_baslik.text = "Kârım"

            self.kar_tutar.text = (
                f"{tl_bicimle(karim)} TL"
            )

            self.kar_tutar.color = KAR_YAZI
            ...
            self.kar_alt_satir.color = KAR_YAZI
