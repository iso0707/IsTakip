...
# Buton rengi seçenekleri için renk tanımları
BUTON_RENKLERI = {
    "turuncu": TURUNCU,
    "sari": SARI,
    "kirmizi": KIRMIZI,
...
# bu koyu mavi (1A43BF) rengi kullanıyor. Menünün arka planı
# ve giriş kutuları (FORM_GIRIS) kemik rengiyle değişmeden kalır.
KOYU_MAVI_BUTON = (0.1020, 0.2627, 0.7490, 1)
...
# değişmeden kalır.
FORM_GIRIS = KEMIK
FORM_GIRIS_CIZGI = (0, 0, 0, 0.16)
FORM_GIRIS_YAZI = SIYAH
FORM_GIRIS_ONER = (0, 0, 0, 1)
...
def buton(
    ...
    if renk is None:
        ...
        # Koyu mavi zemin üzerinde yazı beyaz olmalı.
        ekstra["color"] = BEYAZ
...
class Ayarlar(Screen):
    ...
    def __init__(
        ...
        **kwargs
        ...
        buton_rengi.bind(
            on_press=self.buton_rengi_sec
        )

        ana.add_widget(buton_rengi)
    ...
    def buton_rengi_sec(
        ...
        # Renk seçenekleri butonları
        renk_butonlari = BoxLayout(
            orientation="vertical",
            spacing=dp(8)
        )
        ...
        for renk_kodu, renk_adi in renk_isimleri.items():
            btn = buton(
                renk_adi,
                renk=BUTON_RENKLERI[renk_kodu],
                yukseklik=50,
                font=18
            )

            btn.bind(
                on_press=lambda _, rk=renk_kodu: self.buton_rengi_uygula(rk)
            )

            renk_butonlari.add_widget(btn)

        kutu.add_widget(renk_butonlari)
    ...
    def buton_rengi_uygula(
        self,
        renk_kodu
    ):
        # Seçilen rengi ayarlar dosyasına kaydet
        ayarlar = ayarlari_oku()
        ayarlar["buton_rengi"] = renk_kodu
        ayarlari_kaydet(ayarlar)

        # Koyu mavi buton rengini güncelle
        global KOYU_MAVI_BUTON
        KOYU_MAVI_BUTON = BUTON_RENKLERI[renk_kodu]

        # Form seçici rengini de güncelle
        global FORM_SECICI
        FORM_SECICI = BUTON_RENKLERI[renk_kodu]

        self.durum.text = f"✅ Buton rengi {renk_kodu} olarak ayarlandı."
...
class IsTakipApp(App):
    ...
    def build(self):
        ...
        # Ayarlardan buton rengini oku ve uygula
        ayarlar = ayarlari_oku()
        buton_rengi_kodu = ayarlar.get("buton_rengi", "turuncu")
        
        if buton_rengi_kodu in BUTON_RENKLERI:
            global KOYU_MAVI_BUTON, FORM_SECICI
            KOYU_MAVI_BUTON = BUTON_RENKLERI[buton_rengi_kodu]
            FORM_SECICI = BUTON_RENKLERI[buton_rengi_kodu]
