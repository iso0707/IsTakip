...
# Popup pencerelerinin ortak koyu zemin rengi. HATA
# DÜZELTMESİ: Bazı Popup'larda (İş Yeri Seç, Yeni İş Yeri,
# Malzeme Ekle/Sil vb.) background_color/title_color hiç
# ayarlanmadığı için Kivy'nin varsayılan AÇIK renkli zemini
# üstünde varsayılan BEYAZ başlık yazısı kullanılıyordu; bu da
# başlığın (ve bazı içeriklerin) okunmaz hale gelmesine yol
# açıyordu. Artık her Popup bu koyu zemini ve beyaz başlığı
# kullanıyor.
POPUP_ARKA_PLAN = (0.09, 0.11, 0.145, 1)

YESIL = (0.20, 0.65, 0.30, 1)
KIRMIZI = (0.85, 0.20, 0.20, 1)
SARI = (0.96, 0.78, 0.10, 1)
SARI_METIN = (0.10, 0.08, 0.02, 1)
TURUNCU = (1.0, 0.5, 0.0, 1)

# "Yeni İş" ve "İş Detayı" ekranlarında, kullanıcının veri
# girdiği/eklediği butonlar (İş Yeri Seç, Malzeme/Yer Ekle vb.)
# için kemik rengi. Durum, Kaydet ve Tahsilat gibi
# butonlar/kutular bu renge dahil edilmez.
KEMIK = (0.89, 0.855, 0.788, 1)

# "Yeni İş" / "İş Detayı" ekranlarındaki iş menüsü butonları
# (İş Yeri Seç, Malzeme/Yer Ekle vb.) artık kemik rengi yerine
# bu gri (8A8A8A) rengi kullanıyor. Menünün arka planı kemik
# rengiyle değişmeden kalır.
KOYU_MAVI_BUTON = (0.541, 0.541, 0.541, 1)  # 8A8A8A

# ---- ANA MENÜ (yeni tasarım) ----
KOYU_ZEMIN = (0.055, 0.075, 0.10, 1)

# Sadece "Yeni İş" / "İş Detayı" (iş menüsü) ekranlarının
# zemini için zümrüt yeşili. Ana sayfa (AnaSayfa) hâlâ
# KOYU_ZEMIN kullanmaya devam eder, etkilenmez.
ISS_MENU_ZEMIN = (0.0, 0.31, 0.22, 1)
...
# değişmeden kalır.
FORM_GIRIS_ESKI_VARSAYILAN = (0.949, 0.949, 0.949, 1)  # F2F2F2 - eski varsayılan (artık kullanılmıyor, seçenek listesinde duruyor)

# İstek üzerine uygulama ilk indiğinde (ayarlar.json henüz
# yokken) kutu rengi SİYAH olarak başlıyor.
FORM_GIRIS_VARSAYILAN = SIYAH

# FORM_GIRIS artık sabit değil; ayarlar.json'da kayıtlı bir
# renk varsa (bkz. AYARLAR / KUTU / YAZI RENGİ bölümü) o
# yüklenir, yoksa yukarıdaki varsayılan (siyah) kullanılır.
# Bu değişken kutu_rengini_uygula() ile çalışma anında da
# güncellenir.
FORM_GIRIS = FORM_GIRIS_VARSAYILAN

FORM_GIRIS_CIZGI = (1, 1, 1, 0.18)

# Kutunun içine YAZILAN (hint değil, gerçek) metnin rengi.
# Varsayılan kutu rengi artık siyah olduğu için, yazılan
# metin de siyah zeminde görünsün diye beyaz yapıldı.
FORM_GIRIS_YAZI = BEYAZ

# Hint (yer tutucu) yazı rengi. Artık sabit değil; kutu rengi
# gibi ayarlar.json'dan yüklenir ve yazi_rengini_uygula() ile
# çalışma anında güncellenebilir. İstek üzerine uygulama ilk
# indiğinde TURUNCU (tam opak) olarak başlıyor; Ayarlar
# ekranından (YAZI_RENK_SECENEKLERI) istenilen renge
# değiştirilebilir.
FORM_GIRIS_ONER_VARSAYILAN = TURUNCU
FORM_GIRIS_ONER = FORM_GIRIS_ONER_VARSAYILAN

# Kullanıcının Ayarlar ekranından seçebileceği hazır kutu
# renkleri. Anahtar: ekranda görünen isim, değer: RGBA.
KUTU_RENK_SECENEKLERI = {
    "Siyah (Varsayılan)": FORM_GIRIS_VARSAYILAN,
    "Beyaz": FORM_GIRIS_ESKI_VARSAYILAN,
    "Koyu Gri": (0.30, 0.30, 0.30, 1),
    "Açık Gri": (0.80, 0.80, 0.80, 1),
    "Açık Sarı": (1.0, 0.95, 0.70, 1),
    "Açık Mavi": (0.80, 0.88, 1.0, 1),
    "Açık Yeşil": (0.80, 0.95, 0.82, 1),
    "Açık Turuncu": (1.0, 0.87, 0.70, 1),
    "Açık Pembe": (1.0, 0.82, 0.86, 1),
}

# Hint yazısı için hazır renk seçenekleri.
YAZI_RENK_SECENEKLERI = {
    "Turuncu (Varsayılan)": FORM_GIRIS_ONER_VARSAYILAN,
    "Beyaz": BEYAZ,
    "Siyah": SIYAH,
    "Koyu Gri": (0.25, 0.25, 0.25, 1),
    "Kırmızı": KIRMIZI,
    "Mavi": (0.10, 0.30, 0.75, 1),
    "Yeşil": YESIL,
}

# Ekranda o an var olan, hint'e sahip tüm giriş kutularının
# canvas Color talimatlarını burada tutuyoruz. Kutu rengi
# ayarlardan değiştirildiğinde uygulamayı yeniden başlatmadan
# hepsini anında güncelleyebilmek için kullanılır.
_KUTU_RENK_KAYITLARI = []

# Aynı mantıkla, hint YAZI rengini anında güncelleyebilmek
# için bu sefer widget'ların kendisini (Color talimatı değil,
# çünkü hint_text_color widget property'si) tutuyoruz.
_HINT_YAZI_KAYITLARI = []


def kutu_rengini_uygula(yeni_renk, kaydet_json=True):

    # Çalışma anındaki tüm kutuları anında günceller ve
    # istenirse (varsayılan: evet) ayarlar.json'a yazar.
    # Herhangi bir adımda hata olursa (ör. json yazılamazsa)
    # exception olduğu gibi çağırana fırlatılır; Ayarlar
    # ekranındaki buton bunu yakalayıp ekranda gösterir.

    global FORM_GIRIS

    FORM_GIRIS = tuple(yeni_renk)

    for renk_talimati in list(_KUTU_RENK_KAYITLARI):

        try:
            renk_talimati.rgba = FORM_GIRIS
        except ReferenceError:
            _KUTU_RENK_KAYITLARI.remove(renk_talimati)

    if kaydet_json:

        mevcut = ayarlari_oku()

        mevcut["giris_kutu_rengi"] = list(FORM_GIRIS)

        ayarlari_kaydet(mevcut)


def kutu_rengini_yukle():

    # Uygulama açılışında ayarlar.json'da kayıtlı bir kutu
    # rengi varsa onu uygular; yoksa varsayılan renkte kalır.

    mevcut = ayarlari_oku()

    kayitli_renk = mevcut.get("giris_kutu_rengi")

    if kayitli_renk and len(kayitli_renk) == 4:
...
def yazi_rengini_uygula(yeni_renk, kaydet_json=True):

    # kutu_rengini_uygula() ile aynı mantık, sadece hint
    # YAZI rengi için. Widget'lar canvas talimatı değil,
    # doğrudan kendi hint_text_color property'lerini
    # güncelliyor. Bir widget artık yoksa (ReferenceError)
    # kayıttan siliniyor; başka türlü bir hata olursa
    # (ör. json yazma hatası) olduğu gibi yukarı fırlatılır.

    global FORM_GIRIS_ONER

    FORM_GIRIS_ONER = tuple(yeni_renk)

    for widget in list(_HINT_YAZI_KAYITLARI):

        try:

            widget.hint_text_color = FORM_GIRIS_ONER

            _hint_yeniden_ciz(widget)

        except ReferenceError:
            _HINT_YAZI_KAYITLARI.remove(widget)

    if kaydet_json:

        mevcut = ayarlari_oku()

        mevcut["hint_yazi_rengi"] = list(FORM_GIRIS_ONER)

        ayarlari_kaydet(mevcut)


def _hint_yeniden_ciz(widget):

    # HATA DÜZELTMESİ (ASIL SEBEP BULUNDU): Kivy'nin TextInput'u
    # hint metnini yeniden çizmeyi `hint_text` değiştiğinde
    # ERTELENMİŞ (bir sonraki kareye planlanan, "debounce"
    # edilmiş) bir iç tetikleyiciyle yapıyor. Önceki denemelerde
    # hint_text'i AYNI KARE İÇİNDE (senkron) önce değiştirip
    # hemen eski haline geri döndürüyorduk. Kivy'nin ertelenmiş
    # tetikleyicisi asıl işini yapmaya (bir sonraki karede)
    # başladığında hint_text zaten eski değerine dönmüş
    # oluyordu; birden fazla tetikleme tek bir ertelenmiş
    # çağrıya birleştiği (coalesce) için Kivy "hint_text zaten
    # aynı, değişiklik yok" diye algılayıp yeniden çizimi hiç
    # yapmıyordu. Bu YÜZDEN renk hiç değişmiyordu.
    #
    # ÇÖZÜM: değiştirme ve geri alma işlemini FARKLI karelere
    # yayıyoruz. Önce hint_text'i gerçekten geçici bir değere
    # değiştiriyoruz (bu kare işleniyor, gerçek bir yeniden
    # çizim oluyor), SONRA (bir sonraki karede) asıl hint
    # metnine -artık güncel renkle- geri dönüyoruz. Böylece
    # Kivy'nin tetikleyicisi araya gerçek bir durum değişikliği
    # görüyor ve atlamıyor.

    try:
        _onceki_hint = widget.hint_text
    except Exception:
        return

    _gecici_hint = "" if _onceki_hint else " "

    def _adim1(dt, w=widget, gecici=_gecici_hint):
        try:
            w.hint_text = gecici
        except Exception:
            pass

    def _adim2(dt, w=widget, orijinal=_onceki_hint):
        try:
            # Rengi burada da tekrar atıyoruz; araya başka bir
            # renk değişikliği girmiş olsa bile en güncel renk
            # kullanılsın diye global'den okuyoruz.
            w.hint_text_color = FORM_GIRIS_ONER
            w.hint_text = orijinal
        except Exception:
            pass

    Clock.schedule_once(_adim1, 0)
    Clock.schedule_once(_adim2, 0.12)


def yazi_rengini_yukle():

    # Uygulama açılışında ayarlar.json'da kayıtlı bir hint
    # yazı rengi varsa onu uygular; yoksa varsayılan (siyah)
    # renkte kalır.

    mevcut = ayarlari_oku()

    kayitli_renk = mevcut.get("hint_yazi_rengi")
...
# "İş Yeri Seç" butonu ve "Durum" seçicisi, sıradan metin
# giriş kutularından ve diğer butonlardan görsel olarak
# ayrılsın diye ortak, canlı bir turuncu vurgu rengi
# kullanır. İkisi de bu rengi paylaşır (aynı "tür"), böylece
# tek bakışta "buraya dokunup seçim yapılır" olduğu anlaşılır;
# koyu zeminli normal alanlarla karışmaz.
FORM_SECICI = TURUNCU
FORM_SECICI_YAZI = (0.06, 0.10, 0.11, 1)
    ...
    def __init__(
        ...
        **kwargs
        ...
        super().__init__(
            ...
            hint_text_color=FORM_GIRIS_ONER,
            cursor_color=GIRIS_METIN,
            **kwargs
        )

        # Hint rengi sabit beyaz DEĞİL; diğer giriş kutularıyla
        # aynı şekilde ayarlardan (Turuncu vb.) besleniyor ve
        # Ayarlar ekranından anında güncellenebilmesi için
        # kayıt listesine ekleniyor.
        self.hint_text_color = FORM_GIRIS_ONER

        _HINT_YAZI_KAYITLARI.append(self)
...
def buton(
    ...
    if renk is None:
        ...
        # Kemik rengi zemin açık olduğu için üstündeki yazı
        # tam siyah olmalı (BUTON_METIN'den bile daha net).
        ekstra["color"] = SIYAH

    elif renk == KOYU_MAVI_BUTON:

        # Zemin artık çok açık gri (F1F2F7) olduğu için
        # yazı siyah olmalı.
        ekstra["color"] = SIYAH
...
def giris(
    ...
    # Hint rengi sabit beyaz DEĞİL; diğer giriş kutularıyla aynı
    # şekilde ayarlardan (Turuncu vb.) besleniyor ve Ayarlar
    # ekranından anında güncellenebilmesi için kayıt listesine
    # ekleniyor.
    kutu.hint_text_color = FORM_GIRIS_ONER

    _HINT_YAZI_KAYITLARI.append(kutu)
...
class TextInputKoyu(TextInput):
    ...
    def __init__(self, **kwargs):
        ...
        # NOT: Bazı Kivy sürümlerinde hint_text_color'ı
        # sadece constructor kwarg'ı olarak vermek, widget ilk
        # oluşturulduğunda hint metnini hâlâ varsayılan (gri/
        # beyaz) renkte çizebiliyor. Değeri __init__'ten SONRA
        # tekrar açıkça atayarak Kivy'nin ilgili Property
        # tetikleyicisini (redraw) zorluyoruz; bu da hint
        # metninin gerçekten FORM_GIRIS_ONER (siyah) rengiyle
        # çizilmesini garantiler.
        self.hint_text_color = FORM_GIRIS_ONER

        # Yazı (hint) rengi ayarlardan değiştirildiğinde bu
        # kutunun da anında güncellenebilmesi için kayıt
        # listesine ekleniyor.
        _HINT_YAZI_KAYITLARI.append(self)
        ...
        with self.canvas.before:
            ...
            self._form_zemin_renk = Color(*FORM_GIRIS)

            # Kutu rengi ayarlardan değiştirildiğinde bu
            # kutunun da anında güncellenebilmesi için
            # kayıt listesine ekleniyor.
            _KUTU_RENK_KAYITLARI.append(
                self._form_zemin_renk
            )
...
class SpinnerOptionKoyu(SpinnerOption):
    # Bir Spinner'a (Durum/Kategori/Birim vb.) dokunulduğunda
    # açılan listedeki HER BİR seçenek satırı için kullanılır.
    #
    # HATA DÜZELTMESİ: Spinner'ın kendisi (kapalıyken görünen
    # kutu) koyu temaya boyanmış olsa bile, açılır listedeki
    # seçenekler Kivy'nin VARSAYILAN (açık renk zeminli, beyaz
    # yazılı) stilini kullanmaya devam ediyordu; bu da "menüdeki
    # yazılar beyaz, okunmuyor" sorununa yol açıyordu. Burada
    # açılır listedeki satırlar da formun koyu temasıyla
    # (koyu zemin + açık renk yazı) eşleştirilir.
    def __init__(self, **kwargs):
        kwargs["background_normal"] = ""
        kwargs["background_down"] = ""
        kwargs["background_color"] = FORM_GIRIS
        kwargs["color"] = FORM_GIRIS_YAZI
        super().__init__(**kwargs)
...
def spinner_koyu_stili(spinner):
    ...
    # Hem kapalıyken görünen kutuyu HEM DE açılır listedeki
    # seçenekleri (option_cls) koyu temaya çevirir.
    spinner.background_normal = ""
    spinner.background_color = FORM_GIRIS
    spinner.color = FORM_GIRIS_YAZI
    spinner.option_cls = SpinnerOptionKoyu
    return spinner
...
class YeniIs(Screen):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Yeni tasarım: ekran zemini zümrüt yeşili (iş
        # menüsüne özel), diğer ekranlar etkilenmeden
        # sadece bu ekrana özel çizilir.
        with self.canvas.before:

            Color(*ISS_MENU_ZEMIN)
        ...
        self.gorusme_tarihi = TarihInput(
            hint_text="Görüşme / Anlaşma tarihi - dokun ve seç",
            size_hint_y=None,
            height=dp(58),
        ...
        # İş yeri seç butonu için yuvarlatılmış köşeler
        with yer_btn.canvas.before:
            Color(*TURUNCU)
            yer_btn._arka = RoundedRectangle(
                pos=yer_btn.pos,
                size=yer_btn.size,
                radius=[dp(10)]
            )
        ...
        yer_btn.bind(
            pos=lambda *args: setattr(yer_btn._arka, 'pos', yer_btn.pos),
            size=lambda *args: setattr(yer_btn._arka, 'size', yer_btn.size)
        )
        ...
        self.durum = Spinner(
            ...
            bold=True,
            background_normal="",
            background_color=FORM_SECICI,
            color=FORM_SECICI_YAZI,
            option_cls=SpinnerOptionKoyu
        )
    ...
    def yer_sec(
        ...
        popup = Popup(
            ...
            content=kutu,
            size_hint=(.90, .84),
            background_color=POPUP_ARKA_PLAN,
            title_color=BEYAZ,
            separator_color=SARI
        )
...
class IsDetay(Screen):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Yeni tasarım: YeniIs ile aynı zümrüt yeşili zemin.
        with self.canvas.before:
...
class Ayarlar(Screen):
    ...
    def __init__(
        ...
        **kwargs
        ...
        kutu_rengi.bind(
            on_press=self.kutu_yazi_rengi_sec
        )

        ana.add_widget(kutu_rengi)
    ...
    def kutu_yazi_rengi_sec(
        ...
        for isim, renk in KUTU_RENK_SECENEKLERI.items():
            ...
            secenek_btn.bind(
                on_press=lambda x, r=renk, i=isim:
                self.kutu_rengi_secildi(r, i)
            )

            kutu.add_widget(secenek_btn)

        kutu.add_widget(
            Label(
                text="YAZI (HİNT) RENGİ",
                font_size=16,
                bold=True,
                color=SARI,
                size_hint_y=None,
        ...
        for isim, renk in YAZI_RENK_SECENEKLERI.items():
    ...
    def kutu_rengi_secildi(
        ...
        try:
            ...
            self.durum.text = (
                "❌ Kutu rengi değiştirilemedi.\n"
                f"Hata türü: {type(e).__name__}\n"
                f"Detay: {e}"
            )

    def yazi_rengi_secildi(
        self,
        renk,
        isim
    ):
...
class IsTakipApp(App):
    ...
    def build(self):
        ...
        # Kayıtlı giriş kutusu rengi ve hint yazı rengi
        # varsa (ayarlar.json), ekranlar (ve içindeki hint
        # kutuları) oluşturulmadan önce uygulanır.
        kutu_rengini_yukle()
        yazi_rengini_yukle()
