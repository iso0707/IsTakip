...
#   1) Her gün saat 10:00 -> "Merhaba Ustam! İşler nasıl
#      gidiyor ?" bildirimi
...
_BILDIRIM_KANAL_ADI = "Bi Tıkla Hesap Hatırlatmaları"
...
def _android_bildirim_gonder(baslik, mesaj):
    ...
    # NOT: "android.os.Build" autoclass'ı bazı pyjnius sürümlerinde
    # iç (nested) statik sınıf olan Build$VERSION'ı otomatik
    # attribute olarak açığa çıkarmıyor ("Build.VERSION.SDK_INT"
    # -> "type object 'android.os.Build' has no attribute
    # 'VERSION'" hatası buradan geliyordu). Çözüm: iç sınıfı
    # doğrudan kendi tam adıyla ("Build$VERSION") ayrıca autoclass
    # etmek.
...
def bildirim_gonder(baslik, mesaj):
    ...
    Android'de SADECE yerel (pyjnius) yol kullanılır. ÖNEMLİ:
    Android'de plyer'a ARTIK DÜŞÜLMÜYOR - plyer'ın kendi Android
    bildirim arka ucu, Android 12+ (API 31+) üzerinde PendingIntent
    oluştururken FLAG_IMMUTABLE/FLAG_MUTABLE belirtmediği için
    IllegalArgumentException ile çöküyor (bilinen bir plyer hatası).
    Önceden buraya sessizce düşülüyordu ve kullanıcıya GERÇEK
    hata (bizim kendi kodumuzdaki asıl sebep) değil, plyer'ın bu
    hatası gösteriliyordu. Artık Android'de sadece kendi kodumuz
    kullanılıyor, başarısız olursa GERÇEK sebebi doğrudan
    gösteriyoruz.
    """
    if platform == "android":
        try:
...
def _hata_metni(hata):
    ...
    return (
        ...
        + "\nTelefon Ayarları > Uygulamalar > Bi Tıkla Hesap > "
