...
# (plyer.audio ile). Kaydedilen ses dosyası uygulamanın kendi
# "ses_kayitlari" klasörüne kopyalanır; işe sadece dosya adı
# kaydedilir (isler.json içinde "ses_kayitlari": [...] listesi).
#
...
# eklenmelidir.
#
# NOT: Masaüstünde (Windows/Linux/Mac) plyer.audio çoğu zaman
# desteklenmez; bu durumda hata sessizce yakalanır, kullanıcıya
# kısa bir bilgi mesajı gösterilir, uygulamanın diğer kısımları
# etkilenmez.
...
class _MikrofonKaydedici:
    """
    plyer.audio üzerinden gerçek mikrofon kaydını yönetir.
    plyer nesnesi ilk kullanımda (lazy) alınır; böylece
    masaüstünde import hatası uygulama açılışını etkilemez.
    """
    ...
    def bitir(self):
        ses = self._al()
        ses.stop()
        return getattr(ses, "file_path", None)
...
class SesKaydedici(BoxLayout):
    ...
    def _kaydi_bitir(self, *args):
        ...
        try:
            ...
            print(f"[ses] kopyalama hatası: {e}")
            self.durum_label.text = "Kayıt kaydedilemedi."
            return
