...
class _MikrofonOynatici:
    """
    HATA DÜZELTMESİ: Kaydedilen ses dosyalarını (özellikle
    MediaRecorder ile üretilen .m4a dosyalarını) OYNATMAK için
    daha önce Kivy'nin dahili ses motoru
    (kivy.core.audio.SoundLoader) kullanılıyordu. Bu, kayıt
    tarafındaki eski plyer sorununa çok benzer şekilde güvenilir
    değildi: birçok Android cihazda/derlemede SoundLoader.load()
    ya None döndürüyor ya da play() çağrısı sessizce hiçbir şey
    yapmıyordu. Sonuç olarak kullanıcı ses kaydı YAPABİLİYOR
    (MediaRecorder zaten kendi Android API'sini kullandığı için
    çalışıyordu) ama az önce ya da daha önce kaydettiği sesi
    DİNLEYEMİYORDU.

    Kayıtta yapılan düzeltmeyle birebir aynı mantıkla, oynatma da
    artık SoundLoader ÜZERİNDEN DEĞİL, doğrudan Android API'siyle
    (pyjnius ile android.media.MediaPlayer) yapılır. Bu, gerçek
    cihazlarda standart ve güvenilir bir yöntemdir.

    Bitiş anını yakalamak için ayrı bir Java arayüzü
    (OnCompletionListener) uygulamak yerine -bu, buildozer/gradle
    derlemelerinde kırılgan olabildiği için- kısa aralıklarla
    isPlaying() durumu kontrol edilir (basit ve sağlam bir
    yöntem).
    """
    ...
    def __init__(self):
        self._player = None
        self._dosya_yolu = None
        self._bitis_callback = None
        self._izleme_olayi = None

    def oynat(self, dosya_yolu, bitince=None):
        from jnius import autoclass

        MediaPlayer = autoclass("android.media.MediaPlayer")

        self._temizle(cagir=False)

        player = MediaPlayer()

        try:
            player.setDataSource(dosya_yolu)
            player.prepare()
            player.start()
        except Exception:
            try:
        ...
        self._player = player
        self._dosya_yolu = dosya_yolu
        self._bitis_callback = bitince

        self._izleme_olayi = Clock.schedule_interval(
            self._durum_kontrol, 0.4
        )

    def _durum_kontrol(self, dt):

        if self._player is None:
            return False

        try:
            calisiyor = self._player.isPlaying()
        except Exception:
            calisiyor = False

        if not calisiyor:
            self._temizle(cagir=True)
            return False

    def stop(self, *args):
        self._temizle(cagir=False)

    def _temizle(self, cagir):

        if self._izleme_olayi:
            Clock.unschedule(self._izleme_olayi)
            self._izleme_olayi = None
        ...
        if cagir and callable(callback):
            callback()
...
class SesKaydedici(BoxLayout):
    ...
    def yukle(self, dosyalar):
        ...
        # HATA DÜZELTMESİ / GERİYE UYUMLULUK: Eski kayıtlarda
        # (bu özellik eklenmeden önce) "ses_kayitlari" listesi
        # düz dosya adlarından oluşuyordu (["ses_...m4a", ...]).
        # Artık her kayda bir İSİM eklenebildiği için liste
        # elemanları {"dosya": ..., "isim": ...} sözlüğüne
        # dönüştü. Eski (düz metin) kayıtlar bozulmasın diye
        # burada ikisi de kabul edilip aynı biçime çevrilir.
        normal = []

        for oge in (dosyalar or []):
    ...
    def _calma_toggle(self, dosya_adi, buton_widget, *args):
        ...
        if self._calan_ses:
            self._calan_ses.stop()
            if self._calan_buton:
                self._calan_buton.text = "Dinle"
            self._calan_ses = None
            self._calan_dosya = None
            self._calan_buton = None

        dosya_yolu = tam_yol(dosya_adi)
        ...
        if platform == "android":
            ...
            # HATA DÜZELTMESİ: Android'de oynatma artık
            # SoundLoader yerine doğrudan MediaPlayer ile
            # yapılır (bkz. _MikrofonOynatici). Bu sayede
            # kayıtta kullanılan MediaRecorder'ın ürettiği
            # .m4a dosyası güvenilir şekilde çalınabilir.
            ses = _MikrofonOynatici()

            try:
                ses.oynat(dosya_yolu, bitince=_bitince)
            except Exception as e:
                print(f"[ses] oynatma başlatılamadı: {e}")
                self.durum_label.text = (
            ...
            # Masaüstünde (test amaçlı) Kivy'nin kendi ses
            # motoru kullanılmaya devam eder.
            ses = SoundLoader.load(dosya_yolu)
    ...
    def _sil(self, dosya_adi, *args):
        ...
        self.dosyalar = [
            kayit for kayit in self.dosyalar
            if kayit.get("dosya") != dosya_adi
        ]
    ...
    def _listeyi_yenile(self):
        ...
        for i, kayit in enumerate(
            self.dosyalar, start=1
        ):

            dosya_adi = kayit.get("dosya", "")
            ...
            # GÜNCELLEME: Artık sabit bir etiket yerine
            # düzenlenebilir bir isim kutusu var. Kullanıcı
            # kayda tıklayıp bir İSİM yazabilir; boşken
            # kayıt numarası/tarihi ipucu (hint) olarak
            # görünür. Yazılan isim self.dosyalar içindeki
            # ilgili kayda anında kaydedilir (kayit sözlüğü
            # doğrudan değiştirilir).
            isim_girisi = TextInput(
                text=kayit.get("isim", ""),
                hint_text=(
                    f"Kayıt {i}  •  "
                ...
                foreground_color=_BEYAZ,
                hint_text_color=_BEYAZ,
                cursor_color=_BEYAZ,
                font_size=13,
                padding=[dp(10), dp(12)],
                size_hint_x=1
            ...
            isim_girisi.bind(
                text=lambda inst, deger, k=kayit:
                k.__setitem__("isim", deger)
            )
